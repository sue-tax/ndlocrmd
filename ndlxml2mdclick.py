...
def process_main(rawinputpathlist, change_comma):
...
def main():
    ...
    for index in range(1, len(sys.argv)):
        if os.path.isdir(sys.argv[index]):
            # 右クリックされたのがフォルダならば、
            # フォルダ内のすべのファイルを対象とする
            for inputpath in glob.glob( \
                        os.path.join(sys.argv[index],"*")):
                rawinputpathlist.append(inputpath)
        elif os.path.isfile(sys.argv[index]):
            # 右クリックされたのがファイルならば、
            # そのファイルを対象とする
            rawinputpathlist.append(sys.argv[index])
    process_main(rawinputpathlist, change_comma)
